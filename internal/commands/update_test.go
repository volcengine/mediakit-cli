...
func TestUpdateCheckJsonIncludesSkillsStatus(t *testing.T) {
	...
	if status["current"] != "0.1.6" || status["target"] != "0.1.8" || status["in_sync"] != false {
		t.Fatalf("skills_status = %#v", status)
	}
	if status["command"] != "mediakit-cli update" {
		t.Fatalf("skills_status = %#v", status)
	}
}
...
func TestUpdateInstallsTargetSkillsAfterNpmUpdate(t *testing.T) {
	restore := stubUpdateDeps(t, &updatecheck.Result{Current: "0.1.7", Latest: "0.1.8", HasUpdate: true})
	defer restore()
	...
	runSkillsInstallForVersion = func(_ *cobra.Command, version string) error {
		calls = append(calls, "skills:"+version)
		return nil
	}
	...
	if got, want := strings.Join(calls, ","), "npm,skills:0.1.8"; got != want {
	...
	if payload["skills_action"] != "installed" {
		t.Fatalf("payload = %#v", payload)
	}
}
...
func TestUpdateDoesNotRunSkillsInstallAfterNpmUpdateFailure(t *testing.T) {
	restore := stubUpdateDeps(t, &updatecheck.Result{Current: "0.1.7", Latest: "0.1.8", HasUpdate: true})
	defer restore()

	runSkillsInstall = func(*cobra.Command) error { return errors.New("skills should not run") }
	runSkillsInstallForVersion = func(*cobra.Command, string) error { return errors.New("skills should not run") }
...
func TestInjectNoticeIncludesSkillsNoticeWhenOutOfSync(t *testing.T) {
	...
	home := mustHome(t)
	oldVersion := buildinfo.Version
	buildinfo.Version = "0.1.7"
	t.Cleanup(func() {
		buildinfo.Version = oldVersion
	})
	if err := updatecheck.SaveCache(home, &updatecheck.Cache{
		Current:   "0.1.7",
		Latest:    "0.1.7",
		CheckedAt: time.Now(),
		HasUpdate: false,
		Status:    updatecheck.CacheStatusReady,
	}); err != nil {
		t.Fatal(err)
	}
...
func stubUpdateDeps(t *testing.T, result *updatecheck.Result) func() {
	...
	oldCheckFresh := checkNowFresh
	oldNpm := runNpmInstall
	oldSkills := runSkillsInstall
	oldSkillsForVersion := runSkillsInstallForVersion
	checkNowFresh = func() *updatecheck.Result { return result }
	runNpmInstall = func(*cobra.Command) error { return nil }
	runSkillsInstall = func(*cobra.Command) error { return nil }
	runSkillsInstallForVersion = func(*cobra.Command, string) error { return nil }
	return func() {
		checkNowFresh = oldCheckFresh
		runNpmInstall = oldNpm
		runSkillsInstall = oldSkills
		runSkillsInstallForVersion = oldSkillsForVersion
	}
}
